granova.ds <- function(xdata, revc = FALSE, sw = 0.4, ne=.5, labcex = 1, ident = FALSE, pt.lab = NULL,
...
plot(x, y, xlim = c(lwb, upb), ylim = c(lwb,upb), pch = 19, col = 'blue', 
        cex = 1.2, xlab = "", ylab = "", main = main, sub = sub, lwd=1.5)
...
#Dashed lines from points to perpendicular cross line in lower left corner
kp <- (3*lwb+upb)/4
ex <- .008*(upb-lwb)
for(i in 1:n){segments(kp + d[i]/2 + ex,kp - d[i]/2 + ex,x[i],y[i],lty = 3, lwd = .8)
              points(kp + d[i]/2 + ex, kp - d[i]/2 + ex, pch= 3, lwd=1.4, col = 'blue', cex=1.6)
...
legend(x="topleft",legend = list(paste("Mean diff. =", round(mn.diff,2)),"95% CI",paste("t =", round(tstat,2))), 
        pch =c("_","_",""),col=c("red","green3",0), bty = "n")
...
abline(-mn.diff, 1, lwd = 3, col = "red", lty = 3)
points(x, y, pch = 19, col = 'blue', cex = 1.2)
