...
# Note that if the number of contrasts is NOT equal to the number of groups less one (i.e., to df 'among'),
# then the argument 'npg' must be set equal to the number in each group (presumed equal for all groups);
...
std.contr <- function(cont, tol = sqrt(.Machine$double.eps)^0.6) {
        ...
        if(sum(abs(colMeans(cont))) > tol){stop("Input vector/matrix must have mean zero (for each column)")}
        if(ncol(cont) == 1){cont <- matrix(cont, ncol = 1)}
        dg <- apply(abs(cont), 2, sum)
        if(length(dg) == 1){dg <- as.matrix(dg)}
...
N <- length(resp)
xind <- indic(vn)
if(!is.matrix(con)){con <- as.matrix(con)}
...
mnrsp<-mean(resp)
abline(h=mnrsp,lty=3,lwd=.7,col='dark red')
mns.cgps.i<-c(mean(resp[Xconss[, i] < 0.]),
mean(resp[Xconss[, i] > 0]))
mns.cgps[i,]<-mns.cgps.i
segments(mean(Xconss[, i][Xconss[, i] < 0]), mean(resp[Xconss[, i] < 0.]), mean(Xconss[,i][Xconss[, i] > 0]), mean(resp[Xconss[, i] > 0]),
    lwd = 2, lty = 6, col = 4)
points(mean(Xconss[, i][Xconss[, i] < 0]), mean(resp[Xconss[, i] < 0.]), pch = 1, cex = 2, col=4)
points(mean(Xconss[, i][Xconss[, i] > 0]), mean(resp[Xconss[, i] > 0.]), pch = 1, cex = 2, col=4)
if(i==4 || i==8 ){print('Examine contrast plots & consider printing')
     pause()}
     }
